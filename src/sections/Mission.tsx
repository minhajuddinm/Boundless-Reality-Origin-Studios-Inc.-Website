...
const stats = [
  ...
  { value: '97%', label: 'AAA games with no meaningful blind accessibility', color: 'from-violet-500/20 to-transparent' },
  { value: '1st',  label: 'Studio building VR games designed blind-first from day one', color: 'from-emerald-500/15 to-transparent' },
]

function AnimatedStat({ stat, index, shouldAnimate }: { stat: typeof stats[0]; index: number; shouldAnimate: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Shine */}
      <div className="absolute inset-0 bg-card-shine opacity-60" aria-hidden="true" />
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={motionEnabled ? { y: bgY, background: 'radial-gradient(ellipse, rgba(0,229,255,0.06) 0%, transparent 70%)' } : { background: 'radial-gradient(ellipse, rgba(0,229,255,0.06) 0%, transparent 70%)' }}
        aria-hidden="true"
      />
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
        ...
        <div>
          ...
          <motion.h2
            id="mission-heading"
            className="text-4xl sm:text-5xl font-black tracking-tight text-bros-white leading-[1.05] mt-6 mb-6"
          ...
          {[
            ...
            'At BROS Inc., accessibility is the design constraint that drives every creative decision. When you design for everyone first, you build richer spatial audio, more intuitive interaction models, and more expressive worlds for every player.',
        ...
        <div className="flex flex-col gap-4">
          {stats.map((stat, i) => (
            <AnimatedStat key={stat.label} stat={stat} index={i} shouldAnimate={should} />
          ))}
        </div>
      </div>
    </section>
